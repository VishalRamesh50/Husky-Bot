...
from ids import (NOT_REGISTERED_ROLE_ID, NOT_REGISTERED_CHANNEL_ID,
    WELCOME_CHANNEL_ID, V_MONEY_ID, SUPERSECSEE_ID, SHWIN_ID,
    RULES_CHANNEL_ID, COURSE_REGISTRATION_CHANNEL_ID)
...
class Onboarding(commands.Cog):
    ...
    async def on_member_join(self, member: discord.Member) -> None:
        """
        When a new member joins the server, sends an embedded message in the welcome channel
        with their profile picture and a welcome message.
        ...
        NOT_REGISTERED_CHANNEL: discord.TextChannel = self.client.get_channel(NOT_REGISTERED_CHANNEL_ID)
        RULES_CHANNEL: discord.TextChannel = self.client.get_channel(RULES_CHANNEL_ID)
        WELCOME_CHANNEL: discord.TextChannel = self.client.get_channel(WELCOME_CHANNEL_ID)
        ...
        # send DM to user
        join_msg: str = (f"Welcome to the **{guild}** server {member.mention}!\n"
